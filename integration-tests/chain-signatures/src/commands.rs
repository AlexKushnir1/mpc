...
pub fn sing_command(contract_id: &AccountId, caller_id: &AccountId) -> String {
...
pub fn respond_command(contract_id: &AccountId, caller_id: &AccountId) -> String {
    let payload_hashed = web3::signing::keccak256(&PAYLOAD);

    let request = SignatureRequest::new(
        Scalar::from_bytes(payload_hashed).unwrap().into(),
        &caller_id,
        "test",
    );
...
pub fn join_command(contract_id: &AccountId, caller_id: &AccountId) -> String {
    let url = "http://localhost:3030";
    let (_, cipher_pk) = hpke::generate();
    let sign_pk = PublicKey::from_str(SIGN_PK).unwrap();
...
pub fn proposed_updates_command(contract_id: &AccountId, caller_id: &AccountId) -> String {
    let args = ProposeUpdateArgs {
        code: None,
        config: Some(Config::default()),
    };

    let borsh_args = borsh::to_vec(&args).unwrap();
...
pub fn init_command(contract_id: &AccountId, caller_id: &AccountId) -> String {
    let threshold: usize = 1;
    let candidates: Candidates = dummy_candidates();

    let init_json = format!(
        "'{}'",
        serde_json::to_string(&json!({"threshold": threshold, "candidates": candidates})).unwrap()
    );
...
pub fn init_running_command(contract_id: &AccountId, caller_id: &AccountId) -> String {
    let init_running_json = format!(
        "'{}'",
        serde_json::to_string(&json!({"epoch": 0, "participants": Participants::from(dummy_candidates()), "threshold": 2,"public_key": PublicKey::from_str(SIGN_PK).unwrap() })).unwrap()
    );
